							...
							Eigen::VectorXd& out_X, Eigen::VectorXd& out_V)
{
	...
	VectorXd x, v, xp, vp, mx, mv, mxp, mvp;
	x = X;
	v = V;
	EvalDerivative(x, v, xp, vp);
	mx = x + timestep*0.5*xp;
	mv = v + timestep*0.5*vp;
	EvalDerivative(mx, mv, mxp, mvp);


	out_X = X + timestep * mxp;
	out_V = V + timestep * mvp;
}
								...
								Eigen::VectorXd& out_X, Eigen::VectorXd& out_V)
{
	...
	VectorXd x, v, xp, vp, dx, dv, dxp, dvp;
	x = X;
	v = V;
	EvalDerivative(x, v, xp, vp);
	dx = x + timestep*xp;
	dv = v + timestep*vp;
	EvalDerivative(dx, dv, dxp, dvp);


	out_X = x + timestep *0.5* (xp + dxp);
	out_V = v + timestep *0.5* (vp + dvp);
}
